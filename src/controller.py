...
def limit_meca500_pose(pose):
    limits = { # limits for meca position and orientation (can change but these are currently safe within bounds)
        "x": (60, 230), # forward and back position 
        "y": (-70, 70), # left right position 
        "z": (280, 310), # up down position 
        "rx": (-100, 100), # left right orientation *****DOESNT WORK****** (the value is read from the force dimension but the meca doesnt do anything when this value chamges)
